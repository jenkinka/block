...
/* Crypto key */
static char * key = "myinsecurekey123456";
...
static void osu_ramdisk_transfer(struct osu_ramdisk_device * dev,
 ...
 */
static void osu_ramdisk_request(struct request_queue * q){
	struct request * req;
 ...
 */
static int osu_ramdisk_bio_transfer(struct osu_ramdisk_device * dev,
				struct bio *pbio)
{
	int i;
	struct bio_vec * bvec;
 ...
 */
static int osu_ramdisk_transfer_request(struct osu_ramdisk_device *dev, 
			struct request *req)
{
	struct bio *pbio;
	int nsect = 0;
 ...
 */
static void osu_ramdisk_full_request(struct request_queue *q)
{
	struct request *req;
	int sectors_xferred;
 ...
 */
static int osu_ramdisk_make_request(struct request_queue *q, struct bio *pbio)
{
	struct osu_ramdisk_device *dev = q->queuedata;
	int status;
...
static int osu_ramdisk_open(struct block_device *device, fmode_t mode)
{
 ...
 * Release
 */
static int osu_ramdisk_release(struct gendisk *disk, fmode_t mode)
{
 ...
 */
int osu_ramdisk_media_changed(struct gendisk *gd)
{
 ...
 */
int osu_ramdisk_revalidate(struct gendisk *gd)
{
	struct osu_ramdisk_device *dev = gd->private_data;
 ...
 */
void osu_ramdisk_invalidate(unsigned long ldev)
{
 ...
 */
int osu_ramdisk_getgeo(struct block_device * block_device, struct hd_geometry * geo) {
        long size;
 ...
 */
static void setup_device(struct osu_ramdisk_device *dev, int which)
{
...
static int __init osu_ramdisk_init(void)
{
	int i;
	...
	Device = kmalloc(ndevices*sizeof(struct osu_ramdisk_device), GFP_KERNEL);
	if (Device == NULL){
		printk(KERN_ERR "osu_ramdisk_init: Failed to allocate the device\n");
		goto out;
	}
 ...
 */
static void __exit osu_ramdisk_exit(void)
{
	int i;
